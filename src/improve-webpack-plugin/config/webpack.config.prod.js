...
let prodConfig = {
    ...
    rules: [
      {
        ...
        use: [
          ...
          MiniCssExtractPlugin.loader, // 将 CSS 代码从打包结果中提取出来的插件
          'css-loader'
        ]
      }
    ]
  ...
  optimization: {
    minimizer: [
      new TerserWebpackPlugin(), // 启动 minimizer 时，手动配置 JS 文件的压缩
      new OptimizeCssAssetsWebpackPlugin(), // 在MiniCssExtractPlugin的基础上，将独立提取的 CSS 文件内容压缩
    ]
  },
  plugins: [
    /**
     * @desc 可以根据 process.env.NODE_ENV 的值，配置不同的域名
     * const config = {
     *   production: {
     *      BASE_URL: 'production.example.com',
     *   },
     *   development: {
     *      BASE_URL: 'development.example.com',
     *   },
     *   test: {
    ...
    new MiniCssExtractPlugin(), // 将 CSS 代码从打包结果中提取出来的插件
  ]
}
